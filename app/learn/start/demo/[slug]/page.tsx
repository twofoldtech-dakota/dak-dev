...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: page.frontmatter.title,
      description: page.frontmatter.description,
      url: `${siteUrl}/learn/start/demo/${slug}`,
    },
    alternates: { canonical: `/learn/start/demo/${slug}` },
  };
}

export default async function DemoDecodedPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <PageTransition className="min-h-screen pb-16">
      ...
      <div className="lg:grid lg:grid-cols-[1fr_220px] lg:gap-10 px-4 sm:px-6 lg:px-0">
        <article className="min-w-0 prose prose-invert prose-lg mdx-content">
          <SectionKicker section="Start Here" kicker="Demo, Decoded" color="amber" />
          <CodeBlockWrapper>
            <MDXRemote source={page.content} components={demoComponents} options={mdxOptions} />
          </CodeBlockWrapper>
