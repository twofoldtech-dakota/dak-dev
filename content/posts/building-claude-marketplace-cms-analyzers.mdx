...
Each analyzer is a standalone Claude Code plugin. Teams install only what they need. The plugin architecture enables specialized agents to operate autonomously, similar to [how APL uses multi-agent orchestration](/blog/building-apl-autonomous-coding-agent).
...
**Markdown is powerful**. The entire plugin—agents, commands, rules—is markdown and JSON. No build process, no compilation. Edit and reload. This simplicity enabled APL to [build complex projects entirely autonomously](/blog/how-apl-built-this-blog).
